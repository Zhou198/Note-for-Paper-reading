...
R \qquad \qquad \text{ if } \left|\left<\mathcal{Y}\_j, \mathbf{f}\right>\right|\leq \delta, \forall j\\\\
\arg\max \left<\mathcal{Y}\_j, \mathbf{f}\right> \qquad \text{ if } \left<\mathcal{Y}\_j, \mathbf{f}\right> \geq \delta, \text{ for some } j\\\\
