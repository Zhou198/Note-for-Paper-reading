...
where $\ell$ is a monotonically increasing (Remark: in binary SVMs, this is decreasing since the label multiplied with classfication function is true, which is opposite to this setting.) loss function and $s$ is the penalty for function space. The induced classifier by $\mathbf{f}$ with reject and refine option is defined as, $$\phi(\mathbf(f))=\begin{cases}
R \text{ if } \left|\left<\mathcal{Y}\_j, \mathbf{f}\right>\right|\leq \delta, \forall j\\\\
